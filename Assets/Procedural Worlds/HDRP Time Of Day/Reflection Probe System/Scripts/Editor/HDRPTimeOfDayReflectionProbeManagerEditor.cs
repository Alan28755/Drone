        ...
        private void GlobalPanel(bool helpEnabled)
        {
            ...
            if (renderMode == ProbeRenderMode.Sky)
            {
                ...
                if (probeTimeMode == ProbeTimeMode.TimeTransition)
                {
                    ...
                    EditorGUILayout.LabelField("Current Value: " + m_manager.GetCurrentTransitionCurveValue(), EditorStyles.boldLabel);
                }
        ...
        private void ProbeDataPanel(bool helpEnabled)
        {
...
#if HDPipeline && UNITY_2021_2_OR_NEWER
            if (m_manager.Profile.m_probeTimeMode == ProbeTimeMode.CustomSetTime || m_manager.Profile.m_probeTimeMode == ProbeTimeMode.TimeTransition)
            {
            ...
            for (int i = 0; i < todData.Count; i++)
            {
                ...
                if (todData[i].m_showSettings)
                {
                    ...
                    {
                        EditorGUI.indentLevel++;
                        todData[i].m_ssgiMultiplier = m_editorUtils.Slider("SSGIMultiplier", todData[i].m_ssgiMultiplier, 0f, 15f, helpEnabled);
                        EditorGUI.indentLevel--;
                    }
