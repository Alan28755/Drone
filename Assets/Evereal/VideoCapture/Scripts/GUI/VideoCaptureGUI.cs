    ...
    private void OnGUI()
    {
        ...
        videoCapture.status == CaptureStatus.READY)
      {
        if (GUI.Button(new Rect(10, Screen.height - 60, 150, 50), "Start Capture"))
        {
          ...
          drone.GetComponent<DroneController>().startCapture = true;
        }
      }
      else if (videoCapture.status == CaptureStatus.STARTED)
      {
        if (GUI.Button(new Rect(10, Screen.height - 60, 150, 50), "Stop Capture"))
        {
          ...
          GameObject drone = GameObject.FindWithTag("Player");
          drone.GetComponent<DroneController>().startCapture = false;
        }
      ...
      if (GUI.Button(new Rect(Screen.width - 350, Screen.height - 60, 150, 50), "Quit"))
      {
        Application.Quit();
      }
    }
