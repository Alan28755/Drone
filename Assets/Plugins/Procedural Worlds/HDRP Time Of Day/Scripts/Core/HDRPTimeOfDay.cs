...
namespace ProceduralWorlds.HDRPTOD
{
    ...
    public class HDRPTimeOfDay : MonoBehaviour
    {
        ...
        public bool UseRayTracing
        {
            ...
            set
            {
                if (m_useRayTracing != value)
                {
                    ...
                    HDRPTimeOfDayAPI.ValidateRayTracingEnabled(value);
                }
            }
        }
        ...
        {
            if (Components.m_timeOfDayVolumeComponenets.m_volumetricClouds != null)
            {
                switch (cloudQuality)
                {
        ...
        {
            if (m_timeOfDayProfile.RayTracingSettings.m_rayTraceSSGI || m_timeOfDayProfile.TimeOfDayData.m_useSSGI)
            {
                return true;
            }
            else
        ...
        private void Enable()
        {
            ...
            if (UseRayTracing)
            {
                ...
                HDRPTimeOfDayRayTracingUtils.ApplyRayTracingSettings(Components, TimeOfDayProfile.RayTracingSettings, this);
            }
            else
            {
        ...
        private void ProcessLightSyncOcclusion()
        {
            if (m_useLightSync)
            {
                ...
                bool rtActive = TimeOfDayProfile.RayTracingSettings.IsRTActive();
                foreach (HDRPTimeOfDayLightComponent lightComponent in m_lightComponenets)
                {
                    ...
                    RTLightOptimization rtLightOptimization = lightComponent.SetCullingState(IsDay, player);
                ...
                RTGlobalQualitySettings rtSettings = TimeOfDayProfile.RayTracingSettings.RTGlobalQualitySettings;
                if (rtSettings.UseRTAdditionalLightOptimization && rtActive)
                {
                    HDRPTimeOfDayLightComponent.UpdateRTOptimization(m_rtLightOptimizations, rtSettings);
        ...
        private void UpdateAdvancedLighting(float time, bool isDay, TimeOfDayProfileData data)
        {
            ...
            {
                data.ApplyAdvancedLighting(Components, UseRayTracing, TimeOfDayProfile.RayTracingSettings, isDay, time, m_overrideVolumeData);
            }
        }
        /// <summary>
