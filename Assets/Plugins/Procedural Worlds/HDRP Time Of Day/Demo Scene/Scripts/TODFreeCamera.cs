...
namespace ProceduralWorlds.HDRPTOD
{
    ...
    public class TODFreeCamera : MonoBehaviour
    {
        ...
        public bool m_allowMouseZoomn = true;
        public float m_zoomInOutSpeed = 100f;
        ...
        private void Update()
        {
            ...
            float forwardDirection = Input.GetAxis(m_verticalKey);
            float rightDirection = Input.GetAxis(m_horizontalKey);

            float speed = Time.deltaTime * (Input.GetKey(KeyCode.LeftShift) ? sprintSpeed : moveSpeed);
            float forward = speed * forwardDirection;
            float right = speed * rightDirection;
            ...
            if (m_inputCaptured)
            {
                ...
                {
                    float mouseWheel = Input.GetAxis(m_scrollWheelKey);
                    if (forwardDirection != 0f || rightDirection != 0f)
                    {
                        sprintSpeed += (mouseWheel * m_speedIncreaseValue);
                    ...
                    {
                        if (m_allowMouseZoomn)
                        {
                            float zoom = (mouseWheel * m_zoomInOutSpeed);
